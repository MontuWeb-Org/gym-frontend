...
interface PageProps {
  params: Promise<{ planAssignmentId: string; workoutId: string }>;
}

export default async function WorkoutTemplateDetailPage({ params }: PageProps) {
  const { planAssignmentId, workoutId } = await params;
  ...
  return (
    <div className="flex min-h-[60vh] flex-col items-center justify-center gap-6 p-8 text-center">
      ...
      <div className="space-y-2">
        <h1 className="text-xl font-bold text-foreground">Workout Template Details</h1>
        <p className="text-sm text-muted-foreground max-w-sm">
          Workout template <strong>#{workoutId}</strong> details page is coming soon.
        </p>
      </div>
      <Link
        href={ROUTES.PLANS.TIMELINE(planAssignmentId)}
